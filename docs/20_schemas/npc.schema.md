...
| `services` | list of enum | no | `15_maps_system/MAP_INTERACTABLES.md`; `10_systems/ENHANCEMENT.md`; `10_systems/PERSISTENCE.md` | See Enums; role-service consistency in Validation. `server` — `inn_rest`/`storage`/`coach` trigger server-authoritative state elsewhere (bind point, bank contents, coach fare/economy) that this file does not itself store. |
...
| `enchanter` | A town's enchanter or smithy interior (`docs/WORLD_PLAN.md`) | `services` ⊇ {`enhance`} |
...
| `coach_keeper` | The coachman at a `coach_station` (`15_maps_system/MAP_INTERACTABLES.md` §9; five coach towns) | `services` ⊇ {`coach`}; optional flavor around what is otherwise a self-service station |
| `guide` | Tutorial/help-lean NPC, e.g. near a `main` spawn or a starter town | No forced `services` |
| `handler` | Camp/outpost logistics (e.g. a frontier camp or dungeon-mouth vendor, `docs/WORLD_PLAN.md`) | No forced `services`; `shop`/`services` authored per instance |
...
only) · `coach` (`15_maps_system/MAP_INTERACTABLES.md` §9 `coach_station`,
`15_maps_system/MAP_CONNECTIONS.md` §3; the retired `waygate` service is invalid)
...
1. **Role-service consistency.** `innkeeper` → `services` ⊇ {`inn_rest`}; `enchanter` → ⊇
   {`enhance`}; `banker` → ⊇ {`storage`}; `coach_keeper` → ⊇ {`coach`}. Other roles carry no
...
7. **Service/object co-location (soft).** If `services` includes `inn_rest`/`storage`/`coach`,
   the home map's `interactables` (or, for `coach`, its `portals`) should include a matching
   `inn_bed`/`storage_chest`/`coach_station` object (`20_schemas/map.schema.md`) — a cross-file
...
id: npc_{NNN}
schema: 20_schemas/npc.schema.md
references: [WORLD_PLAN, MAP_INTERACTABLES, ECONOMY] # add PERSISTENCE if services non-empty; add ENHANCEMENT if role: enchanter or services includes enhance; add DEATH_PENALTY if services includes inn_rest; add MAP_CONNECTIONS if role: coach_keeper or services includes coach
name: "{Display Name}"
region: { region_slug }
map: map_{NNN}
role: { merchant|innkeeper|blacksmith|enchanter|banker|quest_giver|coach_keeper|guide|handler|flavor }
# shop:                          # optional
...
- `blacksmith` vs. `enchanter` role boundary: `10_systems/ENHANCEMENT.md` §5 ties the `enhance`
  service's fee to "a town smith interior," while `docs/WORLD_PLAN.md`'s v2 per-town interior lists
  name a "smithy" only at some towns and no dedicated stand-alone "enchanter" interior (the v1
  "Arcane Sanctum" was retired, `memory.md` C6). Whether every bind town needs an enhance-service
  NPC, and which role carries it where no smithy/enchanter interior is named, is unresolved; flagged
  for `docs/WORLD_PLAN.md`/`10_systems/ENHANCEMENT.md`'s owners.
- Millbrook's guild hall/tavern/mayor's-house interiors (`docs/WORLD_PLAN.md`) don't map cleanly
  onto one obvious `role` from the enum above. Phase D may reuse `quest_giver`/`flavor`/`guide` for
  these, or a future revision may add 1–2 roles within the ≤12 cap. Not resolved here.
...
- Whether `banker`/`coach_keeper` NPCs are ever mechanically required (vs. purely optional
