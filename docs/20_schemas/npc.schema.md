...
(inn bed, storage chest; `15_maps_system/MAP_INTERACTABLES.md`) or the coachman's fare dialogue
(coach service, `15_maps_system/MAP_INTERACTABLES.md` §9), priced by `10_systems/ECONOMY.md`. This doc never restates those — it fixes field names, types, the `role`
...
| `services` | list of enum | no | `15_maps_system/MAP_INTERACTABLES.md`; `10_systems/ENHANCEMENT.md`; `10_systems/PERSISTENCE.md` | See Enums; role-service consistency in Validation. `server` — `inn_rest`/`storage`/`coach` trigger server-authoritative state elsewhere (bind point, bank contents, coach fare transactions) that this file does not itself store. |
...
| `enchanter` | Dedicated enchanter-flavored interior (e.g. Mossmere's weaving athenaeum, Verdant R3) or a town's smithy | `services` ⊇ {`enhance`} |
...
| `coachman` | Coach-station town (the 5 Harthmoor stations, `docs/WORLD_PLAN.md` "Harthmoor Coachworks"; coach service per `15_maps_system/MAP_INTERACTABLES.md` §9) | `services` ⊇ {`coach`}; offers the paid station-to-station ride via fare dialogue (fares in shards, `10_systems/ECONOMY.md`) |
| `guide` | Tutorial/help-lean NPC, e.g. near a `main` spawn or a starter town | No forced `services` |
| `handler` | Party-quest entry handler (PQ handler quests `quest_087`–`090`; owner `10_systems/social/PARTY_QUEST.md`) | No forced `services`; `shop`/`services` authored per instance |
...
only) · `coach` (`15_maps_system/MAP_INTERACTABLES.md` §9 coach service,
`15_maps_system/MAP_CONNECTIONS.md` §3 — the paid Harthmoor Coachworks ride; fares
`10_systems/ECONOMY.md`, stations per `docs/WORLD_PLAN.md` "Harthmoor Coachworks")
...
1. **Role-service consistency.** `innkeeper` → `services` ⊇ {`inn_rest`}; `enchanter` → ⊇
   {`enhance`}; `banker` → ⊇ {`storage`}; `coachman` → ⊇ {`coach`}. Other roles carry no
...
7. **Service/object co-location (soft).** If `services` includes `inn_rest`/`storage`, the home
   map's `interactables` should include a matching `inn_bed`/`storage_chest` object; if it
   includes `coach`, the home map's `portals` should include a `coach`-kind portal and its
   `spawn_points` a `coach_stop` (`20_schemas/map.schema.md`,
   `15_maps_system/MAP_INTERACTABLES.md` §9) — a cross-file consistency expectation, not
   enforceable from this file alone.
...
id: npc_{NNN}
schema: 20_schemas/npc.schema.md
references: [WORLD_PLAN, MAP_INTERACTABLES, ECONOMY] # add PERSISTENCE if services non-empty; add ENHANCEMENT if role: enchanter or services includes enhance; add DEATH_PENALTY if services includes inn_rest; add MAP_CONNECTIONS if role: coachman or services includes coach
name: "{Display Name}"
region: { region_slug }
map: map_{NNN}
role: { merchant|innkeeper|blacksmith|enchanter|banker|quest_giver|coachman|guide|handler|flavor }
# shop:                          # optional
#   items: [item_{category}_{NNNN}]
# services: [ { inn_rest|storage|enhance|coach } ]   # optional
dialog:
...
- `blacksmith` vs. `enchanter` role boundary: `10_systems/ENHANCEMENT.md` §5 ties the `enhance`
  ...
  an explicit "smithy" only at Millbrook — Mossmere's weaving athenaeum (Verdant R3) is the
  closest enchanter-flavored interior, and Emberfoot's, Tidewatch Port's, and Cindershelf's lists
  name neither a smithy nor an enchanter. Whether every bind town needs an enhance-service NPC,
  and which role carries it where no smithy/enchanter interior is named, is unresolved; flagged
  for `docs/WORLD_PLAN.md`/`10_systems/ENHANCEMENT.md`'s owners.
- Millbrook's guild hall/tavern/mayor's-house and Mossmere's weaving-athenaeum interiors
...
- Whether `banker` NPCs are ever mechanically required (vs. purely optional flavor standing near
  a self-service `storage_chest`) is not settled by any system doc; this schema treats the role as
  optional — a bind town's bank can function per `15_maps_system/MAP_INTERACTABLES.md` with no NPC
  present at all. The `coachman` is different: v2.2's coach service is offered only through the
  coachman's fare dialogue (`15_maps_system/MAP_INTERACTABLES.md` §9,
  `15_maps_system/MAP_CONNECTIONS.md` §3), so every coach-station map needs one.
