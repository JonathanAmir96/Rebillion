...
| `rows[].rarity_source` | `10_systems/DROPS.md` §5.5 (this doc's own rows): `elite`·`boss` (two rows only; no raid). |
...
```yaml
# illustrative — real instances land in Phase D. mob_011 (Cinder Houndmaster, Emberfoot elite, Lv10)
# is the same worked mob as 20_schemas/monster.schema.md's own example. Shards row: mean_shards_normal(10)
...
**Boss unique rows pattern** (illustrative fragment, `drop_mob_012`, Cindermaw — the Emberfoot boss,
region-order boss `n=1`, owning `item_equip_0201`/`0202` per `docs/ID_REGISTRY.md`'s boss-unique mapping):
...
4. **Owner tier ↔ per-tier shape (hard).** The owner mob's `tier` — read from its ID slot in
   ...
   - **`boss` (§5.3, region bosses e.g. `mob_012` Cindermaw):** `guaranteed` `shards`; `guaranteed`
...
6. **Boss unique refs (hard).** An `item_equip` ref inside the `item_equip_0201`–`0216`
