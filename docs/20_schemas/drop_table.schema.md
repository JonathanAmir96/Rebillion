...
| `references` | list[doc name] | yes | `docs/VALIDATION.md` §3 | Baseline `[DROPS, ITEMS]`; add `ENHANCEMENT` when any row refs an emberstone; add `WORLD_PLAN` for `boss` tables (region-order boss-unique mapping, rule 6). |
...
| `rows[].rarity_source` | enum | **only when `ref` is a pool id** | `10_systems/DROPS.md` §5.5/§6 | `elite`\|`boss` (`raid` reserved for a future-arc raid tier, `10_systems/DROPS.md` §5.5) — selects which §5.5 rarity-weight row instantiates the pooled equip's `rarity`. Forbidden on non-pool rows (Validation). `server`. |
...
| `rows[].rarity_source` | `10_systems/DROPS.md` §5.5 (this doc's own rows): `elite`·`boss` (`raid` reserved, future-arc). |
...
4. **Owner tier ↔ per-tier shape (hard).** The owner mob's `tier` — read from its ID slot in
   ...
   A future-arc raid tier adds its own shape (and the `raid` rarity source) when it lands
...
- **Party loot distribution.** Which party member receives a boss table's rolled rows on a
  party (PQ-entry) kill is `10_systems/social/PARTY.md` §5's; this schema fixes only the table
  shape, not who gets what.
