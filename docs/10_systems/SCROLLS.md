...
**Raid bonus rooms.** The five raid bonus rooms (`10_systems/social/RAID.md` §6.E, `map_325`–
`map_329`) are a named scroll source: every `reactor` there rolls its raid's bonus table, which
...
**6 per room**, authored on `map_325`–`map_329`. That makes the faucet computable and it has not
