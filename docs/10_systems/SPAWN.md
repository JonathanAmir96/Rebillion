...
**Boss respawn decision.** Both the 8 region bosses and the two raid finale bosses
(`10_systems/social/RAID.md`) use arena-entry instancing rather than a long real-world timer:
...
fair. There is no separate raid-boss monster tier (raids reuse region bosses, `10_systems/social/RAID.md`) (Decision Contract C9). The literal mechanism — a true per-party instance
...
The two raid finale arenas (`map_042`, The Cellar Deep; `map_200`, The Mainspring —
`docs/WORLD_PLAN.md`, `10_systems/social/RAID.md`) do not use the zone spawner (§1–§4) at all
— they are single-boss scripted encounters, exempt exactly like regular arenas (§2). When entered as
a raid they are **party-instanced**: entering allocates that encounter to the entering party
...
arenas are also soloable via open (non-raid) entry at reduced reward (`docs/WORLD_PLAN.md`). Mid-fight
...
- Raid finale add-wave count/pacing is not budgeted here — it is authored per-boss in Phase D
