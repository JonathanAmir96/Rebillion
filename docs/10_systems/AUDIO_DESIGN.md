...
- Do the raids (roster: `10_systems/social/RAID.md` §2) need bespoke instance `bgm` distinct
  ...
  `10_systems/social/RAID.md` to confirm or override. `raid_orrery` (added 2026-07-24) makes the
  default cheaper still: its stage maps *are* R10 open dungeons (`RAID.md` §4), so a bespoke
  instance tag there would need an entry-context switch on one `map_NNN`, not just a new tag.
