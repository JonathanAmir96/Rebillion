...
| `legendary` | 0.0015 | rarest pool rolls, the prized boss uniques |
...
The formula extends to any `level`; this arc's monsters top at 42 (`docs/WORLD_PLAN.md`). At the
`10_systems/LEVELING.md` §1 pace (≈ 480 at-level kills/hour), pure-`normal` hunting income
is ≈ `480 · mean_shards_normal(L)` (≈ 8.6 K/h at Lv 10, ≈ 30 K/h at Lv 40);
...
dedicated `fortune` build at the arc top (Lv 42) reads a `fortune_drop_bonus ≈ 12` from STATS §3,
giving `m ≈ 1.12` (+12% rare-loot rate) — the assassin-fantasy payoff of the `flicker`/`dirk`
double-dip (`10_systems/STATS.md` §2.1) without warping the economy; the headroom to the +100%
cap belongs to future-arc stat growth.
...
### 5.4 PQ-instanced finale kills (`10_systems/social/PARTY_QUEST.md` §4)
A party-quest finale boss uses its ordinary **§5.3 boss table** — same monster, same table,
either entry path; **who receives which roll is `10_systems/social/PARTY.md` §5's distribution
rule**. The PQ premium is not a bigger boss table: one-time completion rewards flow through the
PQ handler quests (`PARTY_QUEST.md` §6), and the party simply defeats the boss more reliably
than a solo open-entry attempt. A future-arc **raid tier** (with its own `rarity_source = raid`
row, §5.5, and a token/exchange loop) is deferred — see Open Questions.
...
| `elite` | 55 | 35 | 9 | 1 | 0.2 |
| `boss` | 0 | 30 | 50 | 18 | 2 |
| `raid` (reserved, future-arc raid tier) | 0 | 0 | 40 | 45 | 15 |
...
- A raid-token → raid-gear exchange loop (and its token IDs) is deferred to the future-arc raid
  tier along with the `raid` rarity row (§5.4/§5.5); nothing in this arc's content may use them.
