...
| Job tier (`00_vision/SCOPE.md`, v2 gates per `10_systems/JOBS.md`) | Levels | `pct` lost | Feel |
|---|---|---|---|
| Novice | 1–7 | 0% | Grace band — tutorial safety |
| 1st job | 8–39 | 1% | Barely noticeable |
| 2nd job | 40+ (authored arc tops at 42) | 3% | A real but small setback |
| 3rd job (future arcs) | reserved | 6% | Reserved bracket for the future 3rd-job band on the road to cap 300 |
...
**Valid bind towns** (the 5 towns with an inn interior, `docs/WORLD_PLAN.md` v2): Emberfoot
Village (`map_001`), Millbrook Central (`map_018`), Mossmere (`map_043`), Tidewatch Port
...
ordinary travel — walking the ring or a paid coach/ferry ride, whose rules belong to
...
Regions without their own bind town (Gloomwood, Sunken Depths, Clockwork Ruins) have no local
bind option — a character leveling there simply respawns at whichever town it last rested in,
then travels back in via the normal network.
...
### 5.2 Boss arena deaths (open entry)
...
### 5.3 Party-instance deaths (party quests — `10_systems/social/PARTY_QUEST.md`)
A death inside a party-quest instance (stage maps or the party-instanced finale arena) does not
...
2. The player may **Release** at any time (no forced timer). Releasing applies the §2/§3
   ...
   PQ's entry portal (`10_systems/social/PARTY_QUEST.md` §2) — **not** to the character's bound
   town. This is a deliberate override of §4 scoped to party instances only; it does not change
   the character's stored bind point.
3. From the entry map, the player may walk back into the instance and **re-enter** while the run
   is still live (the party has not wiped and the run has not dissolved). There is no re-entry
   cooldown beyond the walk itself.
4. **Full-party wipe** in the finale (every member fallen, none returned in time): the attempt
   ends, the arena empties, and the party must re-enter to restart the boss at full life.
...
§5.3's release-and-reenter is the only recovery path in party-instanced content, and
field/dungeon/arena deaths have no revive option at all (respawn is the only path).
...
- The 3rd-job bracket's 6% (§2) is reserved ahead of the future arcs that add that band; retune
  when those arcs' `exp` curve detail lands (`10_systems/LEVELING.md`).
- At the true cap (Lv 300, far beyond this arc) there is no `exp` progress to lose; whether any
  cap-adjacent easing band is wanted is deferred until a future arc approaches the cap.
