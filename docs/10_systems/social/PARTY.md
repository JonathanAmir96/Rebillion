# PARTY.md — Party System, Exp Share & Party Instances
...
exp-share split, loot-share modes, the HUD data contract, and the party bookkeeping for
party-instanced content (`10_systems/social/PARTY_QUEST.md` finales). Kill/`exp`
math is `10_systems/LEVELING.md`/`10_systems/COMBAT_FORMULA.md`; drop tagging and table shape are
`10_systems/DROPS.md`; death/release-and-reenter is `10_systems/DEATH_PENALTY.md`; party-scaled
boss math is `10_systems/COMBAT_FORMULA.md` §13.3. This doc consumes all of those and never
...
## 6. Party instances (PQ finales)

The two party-quest finales (`10_systems/social/PARTY_QUEST.md` §4 — the Cellar King and the
Custodian fought through PQ entry) are **party-instanced** (`10_systems/SPAWN.md` §7): entering
allocates the encounter to the entering party alone. A future raid tier reuses this same
machinery when future arcs land (`00_vision/SCOPE.md`).

- **Legal party size: 3–6** (`10_systems/social/PARTY_QUEST.md` §2 owns the floor; the cap is the
  same flat 6 as §1). This fixes `10_systems/COMBAT_FORMULA.md` §13.3's legal `N` range.
...
- **Boss scaling** (`pq_life(N, L)`, fixed damage, the enrage timer) is entirely
  `10_systems/COMBAT_FORMULA.md` §13.3's; not restated here.
- **CC tier** for a PQ-instanced finale boss is the ordinary `boss` row of
  `10_systems/STATUS_EFFECTS.md` §3 — the same monster, the same tier, regardless of entry path.
  Full CC-immunity is reserved for a future-arc raid tier.
- **Death, release, and re-entry** follow `10_systems/DEATH_PENALTY.md` §5.3 exactly (fallen state,
  ...
  - Once Released to the PQ entry map (`10_systems/DEATH_PENALTY.md` §5.3), they fail the
    same-map gate (§4) until they walk back into the instance.

## Server Dependency

Roster membership, HUD plate data, exp/loot arbitration, and party-instance allocation are all
...
character to reach, so no party ever forms, and party quests stay dormant
(`10_systems/social/PARTY_QUEST.md` Server Dependency; both finale bosses remain soloable via
open arena entry).
