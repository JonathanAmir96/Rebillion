...
Owner doc for the two **party quests** (`pq_undervault`, `pq_mainspring`): their level and
party-size gates, how a party starts a run, stage progression, the finale boss and the
solo open-arena path, reward split, and failure/leaver/re-entry bookkeeping. A party quest
...
`docs/WORLD_PLAN.md` (Party quests section) and the IDs by `docs/ID_REGISTRY.md`. Party
...
owns only the party-quest wrapper: gates, stage flow, and how the shared reward is scoped.

## 1. The two party quests
...
|---|---|---|---|---|---|---|---|---|
| `pq_undervault` | Undervault Heist | Lv 15–22 | 3–6 | `map_038`–`map_040` | `map_042` | The Cellar King (`mob_027`) | `millbrook` (r02) | `quest_087`–`quest_088` |
| `pq_mainspring` | Mainspring Trial | Lv 32–40 | 3–6 | `map_195`–`map_197` | `map_200` | The Custodian (`mob_150`) | `clockwork` (r08) | `quest_089`–`quest_090` |
...
handler quests carry the objective text). The handler-quest pair per PQ lives in its home
region's quest block (`docs/ID_REGISTRY.md`, Quests) — this doc references them, it does not
restate their steps.

## 2. Starting a run — entry gates

A party starts a run by talking to the PQ's handler NPC and accepting its handler quest
...
| Gate | Rule |
|---|---|
| Party required | The starter must be in a `10_systems/social/PARTY.md` party — a solo character cannot start a PQ run (the solo path is the open arena, §4). |
...
| Level band | Every entering member's `level` within the PQ's band (Undervault 15–22, Mainspring 32–40). A member outside the band cannot enter; the band matches `docs/WORLD_PLAN.md`. |
...
- **Linear chain.** Each PQ is a fixed sequence of party-instanced stage maps (Undervault
...
- **PQ (party-instanced) entry** — reached by clearing the stage chain (§3); the boss is
  allocated to the entering party alone (`10_systems/SPAWN.md`). This is the full-reward
  path (§5).
- **Open (non-PQ) entry** — the same arena map has an ordinary open boss entrance that a
...
`10_systems/STATUS_EFFECTS.md`'s existing boss default (no party-quest override).

## 5. Rewards & the reduced solo cut

- **Party (full) reward.** A completed PQ run's `exp` and loot follow
  ...
  own completion payout (`10_systems/QUESTS.md`; PQ quests must pay `exp`, they are not
  Rift/no-exp content). This doc adds **no** new split math; it only scopes *when* that split
  fires (on a PQ finale-boss kill and each handler-quest turn-in).
- **Solo (open-arena) reward.** The open non-PQ entry (§4) pays a **reduced** boss reward
  versus the full PQ completion — the intended incentive to run the co-op path. The exact
...
- **Instance allocated to the party.** Entering a PQ (§2) allocates the whole run — stages
...
restated here. This doc fixes only the party-quest bookkeeping layered on top:
...
itself PQ rewards. Party quests are **instanced co-op and server-deferred** like the rest of
`10_systems/social/`: **the interim solo build ships the two PQs' party path present but
dormant** — no second character exists to form the required 3–6 party, so no PQ instance ever
...
- **Per-character reward lockout / cooldown between PQ completions** (e.g. once-per-day, or
...
- The **reduced-reward factor** for the open non-PQ arena entry (§4–§5) is unset — a
...
- The exact split of each handler-quest **pair** (`quest_087`/`088`, `quest_089`/`090`)
  ...
  (`10_systems/QUESTS.md`); this doc only pins the pair-to-PQ mapping.
- Whether party-instanced allocation for the two PQ arenas is homed in `10_systems/SPAWN.md`
