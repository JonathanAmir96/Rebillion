...
- `docs/ID_REGISTRY.md` — reserved blocks: maps 001–200, mobs 001–150 (tier slots), items
  (equip 0001–0300 incl. boss uniques 0201–0230; use 0001–0060 with 16 well-known IDs; etc
...
- `docs/WORLD_PLAN.md` — 12 region sections: map-type allocation per ID, cross-region edges,
  waygate network, spawn-point conventions, monster slot layouts, element mixes, boss seeds,
  NPC/quest/etc blocks
...
- Post-cap progression model (SCOPE.md → LEVELING.md at B; default gear-only).
- Secret maps in completion metrics (SCOPE.md; default no).
- `haste` split into move/attack components? (GLOSSARY.md → STATS.md at B; default combined).
...
- Interiors combat-free confirmation (WORLD_PLAN.md → MAPS_SYSTEM.md at B).
