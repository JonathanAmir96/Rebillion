...
maps (towns, popular fields), plus **true instances** for arena and party-quest content.
...
economy fork occurs. Boss arenas and party-quest finales are already instanced by
`10_systems/SPAWN.md` §3/§7 and `docs/WORLD_PLAN.md` (`pq_undervault` finale `map_042`,
`pq_mainspring` finale `map_200`), so instance workers are a requirement, not a choice.
...
- **Instance workers** — ephemeral map copies allocated to one party for party-quest stages/
